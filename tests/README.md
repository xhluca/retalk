...
- Starts two broker subprocesses on localhost ports **8767 and 8768**
  (fails fast if they are taken).
- All state (broker DBs, user stores) lives in a temporary directory that
...
`tempfile.TemporaryDirectory()`, and if it starts brokers, give them
...
E2EE round-trips, no plaintext at the broker, MITM refusal on tampered
keys, fallback keys and rotation grace windows, key replenishment,
multi-process store sharing, broker migration with surviving sessions,
delivery acks and outbox recovery with duplicate rejection, and the three
signed-request attack defenses (replay, stale timestamp, cross-broker).
